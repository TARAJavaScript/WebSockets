        ...
        if (that instanceof tb) { // called as constructor, create and return tb object instance
            ...
            if (typeof tbClass === 'function') {
                ...
                if (!tbClass.prototype.__tb__) {
                    Object.defineProperty(tbClass.prototype, '__tb__', {
                        value: 'V8',
                        enumerable: true
                    });
            ...
            return new TbSelector(!!arguments[0] ? arguments[0] : undefined, !!arguments[1] ? arguments[1] : undefined);
        ...
        if (!!typeConfigs[type]) { // either *.css or *.js file
            ...
            promise = new tb.Promise(function (resolve, reject) {
                ...
                Object
                    ...
                    .forEach(
                        function (pKey) {
                            element.setAttribute(pKey, tb.parse(typeConfig.attributes[pKey], {
                                src: pFile
                            }));
                        }
                    );
    ...
    tb.webSocket = (function () {
        ...
        function WS(pConfig) {
			...
			try{
	            ...
	            if ( !!that.config['protocols'] ){
					console.log("xxx:", that.config);
					
	                that.websocket = new WebSocket( // jshint ignore:line
	                    that.config.url, 
	                    that.config['protocols']
	                );
	            } else {
					console.log("xxx no protocols:", that.config);
            ...
            that.websocket.onopen = function(ev) {
				console.log( 'open:', ev );
            ...
            that.websocket.onmessage = function(ev) {
				console.log( 'message:', ev );
                that.trigger('message', ev.data);
            };

            that.websocket.onclose = function(ev) {
				console.log( 'close:', ev );
                that.trigger('close', ev);
            };

			console.log( 'socket:', that.websocket );
        ...
        return function (pUrl, pProtocols) {
			...
			if ( !!pProtocols ){
				config.protocols = pProtocols;
			}
			
			console.log('generating WS');
            return new tb(
                WS, 
				config
            );
...
if (typeof module === 'undefined') { // will not work as a module
    ...
    tb.CRUD = function (pConfig) {
        ...
        // default config mixin -> result will be in that.config
        tb.extend(
            that.config, { // default settings, reference only
                'create': {
                    url: '',
                    method: 'POST',
                    success: function (pResult) {
    ...
    tb.CRUD.prototype = (function () {
        ...
        return {
            ...
            'create': function (pParams) {
                ...
                tb.request(
                    tb.extend(
                        o, { // if params given, use microparse to fill them in url
